...
async function s5UdpAssoc(clientTcp) {
  ...
  function createOutSockForFamily(family) {
    ...
    s.on('error', err => log('error', `UDP out(v${family}):`, err.message));
    ...
    s.bind(p, bind, () => {
      const a = s.address();
      log('info', `UDP out(v${family}) → ${a.address}:${a.port} (range ${CONFIG.udpPortRange.min}-${CONFIG.udpPortRange.max})`);
    });
  ...
  // 预先创建两个出口 socket（IPv4 + IPv6），避免首包延迟
    ...
    try {
      ...
      const doSend = (addr) => {
        ...
        if (pktCounts[family] > CONFIG.udpRotateAfter) {
          log('info', `UDP(v${family}) rotate after ${pktCounts[family]} pkts`);
      ...
      if (dstAtyp === ATYP_DOMAIN) {
        // 域名：同时尝试解析 IPv4 和 IPv6，用能解析到的那个
        dnsLookup(dstHost, { all: false })
          .then(r => doSend(r.address))
