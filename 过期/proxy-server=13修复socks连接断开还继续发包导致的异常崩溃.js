...
async function s5UdpAssoc(clientTcp) {
  ...
  // ─── FIX：竞态条件守卫标志 ────────────────────────────────────────────────
  // TCP 连接断开时 cleanup() 会先关闭 relaySock，但此后延迟关闭期内的旧
  // outSock 仍可能收到回包并尝试调用 relaySock.send()，导致
  // ERR_SOCKET_DGRAM_NOT_RUNNING 未捕获异常崩溃。
  // 用此标志在 onMessage 入口处做守卫，彻底阻断该路径。
  ...
  // ─── FIX：过滤 ERR_SOCKET_DGRAM_NOT_RUNNING，避免重复打印已知关闭错误 ──
  relaySock.on('error', err => {
    if (err.code !== 'ERR_SOCKET_DGRAM_NOT_RUNNING') {
      log('error', 'UDP relay:', err.message);
    }
  });
  ...
  function createOutSockForFamily(family, onReady) {
    ...
    const onMessage = (msg, rinfo) => {
      ...
      // ─── FIX：守卫检查，防止在 relaySock 已关闭后仍调用 send() ────────
      // TCP 断开 → cleanup() 关闭 relaySock → relayClosed = true
      // 旧 outSock 延迟关闭期间收到回包时，此处直接丢弃，避免
      // ERR_SOCKET_DGRAM_NOT_RUNNING 未捕获异常导致进程崩溃。
      if (relayClosed || !clientUdpAddr) return;
  ...
  // ─── FIX：cleanup 先置标志再关闭 socket ──────────────────────────────────
  // 必须在 relaySock.close() 之前将 relayClosed 置为 true，
  // 这样即使关闭期间有 onMessage 回调被异步唤醒，也能在入口处被拦截。
  const cleanup = () => {
    relayClosed = true; // 先置标志，再关 socket
