...
function buildS5Reply(rep, host, port) {
  ...
  if (net.isIPv4(host)) {
    ...
    host.split(':').forEach((p, i) => addrBuf.writeUInt16BE(parseInt(p, 16) || 0, i * 2));
  } else {
    atyp = ATYP_IPV4;
  ...
  const process = async () => {
    try {
      ...
      if (state === 'request') {
        ...
        log('info', `SOCKS5 cmd=0x${cmd.toString(16)} → ${host}:${port}`);
...
async function s5UdpAssoc(clientTcp) {
  const bindHost = normalizeHost(CONFIG.host);
  const v6      = net.isIPv6(bindHost) || bindHost === '::' || bindHost === '::1';
  const udpType = v6 ? 'udp6' : 'udp4';

  const relaySock = dgram.createSocket({ type: udpType, reuseAddr: true });
  let clientUdpAddr = null;
  let outSock = null, pktCount = 0;

  function createOutSock() {
    if (outSock) { try { outSock.close(); } catch (_) {} }
    const p = randomInRange(CONFIG.udpPortRange.min, CONFIG.udpPortRange.max);
    const s = dgram.createSocket({ type: udpType, reuseAddr: true });
    s.on('error', err => log('error', 'UDP out:', err.message));
    s.on('message', (msg, rinfo) => {
      ...
      relaySock.send(Buffer.concat([hdr, msg]), clientUdpAddr.port, clientUdpAddr.address,
        err => { if (err) log('error', 'UDP back:', err.message); });
    });
    // 出口 socket 绑定 0.0.0.0/:: 让系统自动选路由出口，不能绑 127.0.0.1（无法路由到公网）
    const outBindHost = v6 ? '::' : '0.0.0.0';
    s.bind(p, outBindHost, () => {
      const a = s.address();
      log('info', `UDP out socket → ${a.address}:${a.port} (range ${CONFIG.udpPortRange.min}-${CONFIG.udpPortRange.max})`);
    });
    pktCount = 0;
    return s;
  }

  outSock = createOutSock();

  relaySock.on('error', err => log('error', 'UDP relay:', err.message));
  relaySock.on('message', (msg, rinfo) => {
    if (!clientUdpAddr) clientUdpAddr = rinfo;
    ...
    try {
      ...
      pktCount++;
      if (pktCount > CONFIG.udpRotateAfter) {
        log('info', `UDP rotate after ${pktCount} pkts`);
        outSock = createOutSock();
      }
      const doSend = addr => outSock.send(data, dstPort, addr,
        err => { if (err) log('error', 'UDP send:', err.message); });
      if (dstAtyp === ATYP_DOMAIN) {
        dnsLookup(dstHost, { all: false }).then(r => doSend(r.address)).catch(e => log('error', 'UDP DNS:', e.message));
      } else {
        doSend(dstHost);
      }
...
function handleHttp(socket, firstChunk) {
  ...
  function tryParse() {
    ...
    if (method === 'CONNECT') {
      ...
      log('info', `HTTP CONNECT → ${host}:${port}`);
      ...
      log('info', `HTTP ${method} → ${host}:${port}${reqPath}`);
      const remote = net.createConnection({ host, port }, () => {
...
server.listen(CONFIG.port, bindHost, () => {
  ...
  log('info', `  调试模式 : ${CONFIG.debug ? '开启 (--debug)' : '关闭'}`);
  log('info', '─'.repeat(60));
  log('info', `  HTTP  : curl -x http://${disp}:${addr.port} https://example.com`);
  log('info', `  SOCKS5: curl --socks5 ${disp}:${addr.port} https://example.com`);
  log('info', `  SOCKS4: curl --socks4 ${disp}:${addr.port} https://example.com`);
  log('info', '─'.repeat(60));
  log('info', `  --host=0.0.0.0  --port=1080  --udp-min=6811  --udp-max=6922  --udp-rotate=5  --debug`);
  log('info', '═'.repeat(60));
  console.log('');
});

['SIGINT', 'SIGTERM'].forEach(sig =>
  process.on(sig, () => { log('info', '关闭中...'); server.close(() => process.exit(0)); })
);