...
async function s5UdpAssoc(clientTcp) {
  ...
  function createOutSockForFamily(family) {
    ...
    if (old2) { try { old2.close(); } catch (_) {} }
    ...
    s.on('message', (msg, rinfo) => {
      dbg(`UDP out(v${family}) recv from ${rinfo.address}:${rinfo.port} (${msg.length}B), clientUdpAddr=${clientUdpAddr ? clientUdpAddr.address+':'+clientUdpAddr.port : 'null'}`);
      ...
      const pkt   = Buffer.concat([hdr, msg]);
      dbg(`UDP relay back: sending ${pkt.length}B to ${clientUdpAddr.address}:${clientUdpAddr.port}`);
      relaySock.send(pkt, clientUdpAddr.port, clientUdpAddr.address, err => {
